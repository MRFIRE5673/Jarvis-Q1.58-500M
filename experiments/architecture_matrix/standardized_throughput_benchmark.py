...
def run_deterministic_benchmark(model, name="Model", batch_size=2, seq_len=512, warmup_steps=20, timed_steps=100, num_trials=5):
    ...
    print(f"  Peak VRAM  : {peak_vram_mb:8.1f} MB")
...
def main():
    print("=" * 80)
    print("STANDARDIZED DETERMINISTIC BENCHMARK: BASELINE vs CANDIDATE B+C+E")
    print(f"Device: {DEVICE} ({torch.cuda.get_device_name(0)})")
    print("=" * 80)
    
    # 1. Benchmark Baseline
    print("\n>>> Instantiating Paper Baseline...")
    base_model = Jarvis(
        vocab_size=50257,
        d_model=1024,
        n_layers=24,
    ...
    # 2. Benchmark Candidate B+C+E
    print("\n>>> Instantiating Candidate B+C+E...")
    bce_cfg = {
    ...
    print(f"Loading candidate weights from: {os.path.basename(bce_ckpt_path)}")
    ...
    print("=" * 80)
    print(f"{'Metric':<25} | {'Paper Baseline':<18} | {'Candidate B+C+E':<18} | {'Delta':<15}")
    print("-" * 80)
    ...
    tp_diff = f"{bce_res['mean_throughput_tok_s'] - base_res['mean_throughput_tok_s']:+.1f} tok/s"
    print(f"{'Throughput (tok/s)':<25} | {tp_base:<18} | {tp_bce:<18} | {tp_diff:<15}")
    
    st_base = f"{base_res['mean_step_time_ms']:.2f} ms"
    st_bce = f"{bce_res['mean_step_time_ms']:.2f} ms"
    st_diff = f"{bce_res['mean_step_time_ms'] - base_res['mean_step_time_ms']:+.2f} ms"
    print(f"{'Step Latency (ms)':<25} | {st_base:<18} | {st_bce:<18} | {st_diff:<15}")
    
    vr_base = f"{base_res['peak_vram_mb']:.1f} MB"
    vr_bce = f"{bce_res['peak_vram_mb']:.1f} MB"
    vr_diff = f"{bce_res['peak_vram_mb'] - base_res['peak_vram_mb']:+.1f} MB"
    print(f"{'Peak Inference VRAM':<25} | {vr_base:<18} | {vr_bce:<18} | {vr_diff:<15}")
    print("=" * 80)
    
    import json
    out_path = os.path.join(ARCH_DIR, "standardized_throughput_report.json")
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump({"baseline": base_res, "candidate_bce": bce_res}, f, indent=2)
