# experiments/architecture_matrix/run_longer_adaptation_bce.py
"""
LONGER ADAPTATION & COMPREHENSIVE GATE/MEMORY DIAGNOSTICS FOR B+C+E
===================================================================
Runs a controlled 500-step adaptation test from the paper-faithful baseline
checkpoint (ckpt_step_0004284_best.pt).
...
4. Full Gate & Memory Diagnostics:
   - Write gate w_t: mean, std, min, max, saturation % (<0.05 or >0.95)
   - Erase gate e_t: mean, std, min, max, saturation % (<0.05 or >0.95)
   - Gamma (decay): mean, std, min, max, effective retention alpha_t
   - Local buffer activation magnitude vs Recurrent activation magnitude
   - Per-head learned fusion gate distribution
5. Training loss stability & Gradient norm tracking
6. Peak VRAM accounting
"""
...
@torch.inference_mode()
def collect_gate_memory_diagnostics(model, dummy_input):
    model.eval()
    w_vals = []
    e_vals = []
    gamma_vals = []
    ...
    for block in model.blocks:
        ...
        # Direct gate distributions
        w_t = torch.sigmoid(attn.write_gate_proj(x_norm)).view(-1).cpu().numpy()
        w_vals.extend(w_t)
        
        e_t = torch.sigmoid(attn.erase_gate_proj(x_norm)).view(-1).cpu().numpy()
        e_vals.extend(e_t)
        
        gam = torch.sigmoid(attn.gamma_raw).view(-1).cpu().numpy()
        gamma_vals.extend(gam)
    ...
    return {
        "write_gate": {
            ...
            "saturated_high_pct": float(np.mean(w_arr > 0.95) * 100.0),
            "saturated_low_pct": float(np.mean(w_arr < 0.05) * 100.0),
        },
        "erase_gate": {
            "mean": float(np.mean(e_arr)),
            "std": float(np.std(e_arr)),
            "min": float(np.min(e_arr)),
            "max": float(np.max(e_arr)),
            "saturated_high_pct": float(np.mean(e_arr > 0.95) * 100.0),
            "saturated_low_pct": float(np.mean(e_arr < 0.05) * 100.0),
        },
        "gamma_decay": {
        ...
        "fusion_gate": {
            "mean": float(np.mean(fuse_arr)),
            "std": float(np.std(fuse_arr)),
            "min": float(np.min(fuse_arr)),
            "max": float(np.max(fuse_arr)),
        }
    }
...
def main():
    print("=" * 85)
    print("LONGER ADAPTATION EXPERIMENT: B+C+E (500 STEPS)")
    ...
    for step in range(total_steps + 1):
        if step in eval_checkpoints:
            ...
            diag = collect_gate_memory_diagnostics(model, dummy_diag)
            vram_mb = torch.cuda.max_memory_allocated(DEVICE) / (1024 * 1024)
            eval_dt = time.perf_counter() - t_eval_0
            
            print(f"\n>>> [CHECKPOINT STEP {step:03d}/{total_steps}] (Eval Time: {eval_dt:.1f}s)")
            ...
            print(f"  Write Gate: Mean = {diag['write_gate']['mean']:.3f} | Saturation: High={diag['write_gate']['saturated_high_pct']:.1f}%, Low={diag['write_gate']['saturated_low_pct']:.1f}%")
            print(f"  Erase Gate: Mean = {diag['erase_gate']['mean']:.3f} | Saturation: High={diag['erase_gate']['saturated_high_pct']:.1f}%, Low={diag['erase_gate']['saturated_low_pct']:.1f}%")
            print(f"  Decay Gamma: Mean = {diag['gamma_decay']['mean']:.3f} +/- {diag['gamma_decay']['std']:.4f}")
            print(f"  Contributions: Local Buffer = {diag['path_contributions']['local_contribution_ratio']*100:.1f}% | Recurrent = {diag['path_contributions']['recurrent_contribution_ratio']*100:.1f}%")
            ...
            ckpt_path = os.path.join(ARCH_DIR, f"ckpt_bce_long_step{step:04d}.pt")
            torch.save({
                "step": step,
    ...
    print("=" * 85)
    print("500-STEP ADAPTATION COMPLETED SUCCESSFULLY")
    print("=" * 85)
    
    out_file = os.path.join(ARCH_DIR, "longer_adaptation_bce_report.json")
    ...
    print(f"\n[OK] Longer adaptation report saved to: {out_file}")
